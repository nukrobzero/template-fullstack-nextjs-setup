...
export default function SubServicePage({ page }: Props) {
  ...
  const handlePageSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.post("/api/dashboard/subService", data);
      router.reload();
    } catch (e) {
      console.error(e);
    }
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await axios.put("/api/dashboard/subService", data);
      router.reload();
    } catch (e) {
      console.error(e);
    }
  ...
  const handelDelete = async (id: string) => {
    try {
      await axios.delete(`/api/dashboard/subService/${id}`);
      router.reload();
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div>
      <div>
        <h1>Hi this Sub service page</h1>
